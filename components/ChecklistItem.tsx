...
const ChecklistItem: React.FC<ChecklistItemProps> = ({ 
  ...
  const getDifficultyColor = (diffValue: string) => {
    if (task.status === 'completed') return 'bg-slate-200 dark:bg-slate-800 text-slate-500 border-slate-300 dark:border-slate-700';
    
    switch(task.difficulty) {
      case 'Easy Start': return 'bg-emerald-100 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 border-emerald-200 dark:border-emerald-900/30';
      case 'Some Weight': 
        if (completedCount > 14) return 'bg-orange-600 text-white border-orange-400 shadow-[0_0_10px_rgba(249,115,22,0.3)]';
        if (completedCount >= 10) return 'bg-orange-100 dark:bg-orange-700/40 text-orange-700 dark:text-orange-200 border-orange-200 dark:border-orange-500/50';
        if (completedCount >= 5) return 'bg-orange-50 dark:bg-orange-800/30 text-orange-600 dark:text-orange-300 border-orange-100 dark:border-orange-600/40';
      ...
      case 'Heavy Weight': return 'bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 border-red-100 dark:border-red-900/30';
  ...
  return (
    <div className={`group rounded-[2rem] border transition-all duration-500 overflow-visible relative ${task.status === 'completed' ? 'bg-slate-50 dark:bg-[#0e0e11] border-slate-200 dark:border-slate-800/60' : 'bg-white dark:bg-[#111214] border-slate-200 dark:border-slate-800 hover:border-emerald-500/30 shadow-2xl shadow-slate-200 dark:shadow-black/20'}`}>
      
      <div className="p-7 flex items-start gap-6 cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
        <div 
          onClick={handleToggle}
          className={`relative flex-shrink-0 w-14 h-14 rounded-2xl border-2 flex items-center justify-center transition-all duration-300 ${task.status === 'completed' ? 'bg-emerald-100 dark:bg-emerald-900/20 border-emerald-300 dark:border-emerald-700/50' : 'border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-black group-hover:border-emerald-500 shadow-xl shadow-slate-200 dark:shadow-black/40'}`}
        >
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none overflow-visible z-50">
            {popups.filter(p => p.type === 'task').map(p => (
              <div 
                key={p.id} 
                className={`absolute font-black animate-float-up pointer-events-none whitespace-nowrap flex flex-col items-center gap-1 ${p.xpAmount > 0 ? (p.critical ? 'text-orange-500 dark:text-orange-400 drop-shadow-md' : 'text-emerald-500 dark:text-emerald-400 drop-shadow-md') : 'text-rose-500 drop-shadow-md'} ${p.critical ? 'text-5xl' : 'text-3xl'}`}
              >
                <div>{p.xpAmount > 0 ? `+${p.xpAmount}` : p.xpAmount} XP</div>
                {p.coinAmount !== 0 && (
                  <div className="text-amber-500 dark:text-amber-400 text-sm font-black flex items-center gap-1">
          ...
          {task.status === 'completed' ? (
            ...
            <div className="flex flex-col items-center">
              <span className="text-[8px] font-black text-emerald-600/50 dark:text-emerald-500/50 uppercase leading-none">Quest</span>
        ...
        <div className="flex-1 relative">
          <div className="flex flex-wrap items-center gap-3 mb-2">
            <h3 className={`font-black text-xl tracking-tight transition-all uppercase ${task.status === 'completed' ? 'text-slate-400 dark:text-slate-400 decoration-slate-600/50' : 'text-slate-900 dark:text-slate-100'}`}>
              {task.title}
            </h3>
          ...
          <p className={`text-sm font-bold leading-relaxed ${task.status === 'completed' ? 'text-slate-500 dark:text-slate-600' : 'text-slate-500'}`}>{task.description}</p>
        </div>
      </div>
      ...
      {isExpanded && (
        <div className="px-7 pb-6 animate-in slide-in-from-top-4 duration-300">
          <div className="space-y-4">
            <h4 className="text-[9px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-[0.3em] px-2 mb-2">Observation Streams</h4>
            ...
            {activePrereqs.map((p, idx) => (
              <div key={p.id} className="flex items-center gap-4 relative overflow-visible group/item">
                <div 
                  ...
                  className={`relative flex-shrink-0 w-8 h-8 rounded-xl border-2 flex items-center justify-center transition-all ${p.label.trim() === '' ? 'border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-900/50 cursor-not-allowed opacity-30' : 'cursor-pointer border-slate-300 dark:border-slate-700 bg-white dark:bg-black group-hover/item:border-emerald-500/50'}`}
                >
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none overflow-visible z-50">
                    {popups.filter(pop => pop.type === 'prereq' && pop.prereqId === p.id).map(pop => (
                      <div 
                        key={pop.id} 
                        className={`absolute font-black animate-float-up pointer-events-none whitespace-nowrap flex flex-col items-center gap-1 ${pop.xpAmount > 0 ? (pop.critical ? 'text-orange-500 dark:text-orange-400 drop-shadow-md' : 'text-emerald-500 dark:text-emerald-400 drop-shadow-md') : 'text-rose-500 drop-shadow-md'} ${pop.critical ? 'text-4xl' : 'text-2xl'}`}
                      >
                        <div>+{pop.xpAmount} XP</div>
                        <div className="text-amber-500 dark:text-amber-400 text-[10px] font-black flex items-center gap-1">+{pop.coinAmount} ₵</div>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="flex-1 flex items-center gap-2">
                  <input 
                    ...
                    className="flex-1 bg-transparent border-b border-slate-200 dark:border-slate-800 focus:border-emerald-500 dark:focus:border-emerald-500 outline-none text-xs font-bold py-2 text-slate-800 dark:text-slate-300 placeholder:text-slate-400 dark:placeholder:text-slate-700 transition-colors"
                  />
                  
                  <div className="flex items-center gap-2">
                    <button 
                      onClick={() => handleAddRequest(p.label)}
                      className={`p-2.5 rounded-xl transition-all shadow-lg ${p.label.trim() === '' ? 'bg-slate-100 dark:bg-slate-700 text-slate-400 dark:text-slate-500 cursor-not-allowed' : 'bg-emerald-500 text-white shadow-emerald-500/20 hover:bg-emerald-400 hover:scale-105 active:scale-95'}`}
                    ...
                    <button 
                      ...
                      className={`p-2.5 rounded-xl transition-all shadow-lg ${p.label.trim() === '' ? 'bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-600 cursor-not-allowed' : 'bg-indigo-600 text-white shadow-indigo-500/20 hover:bg-indigo-500 hover:scale-105 active:scale-95'}`}
                      disabled={p.label.trim() === ''}
            ...
            {completedPrereqs.length > 0 && (
              <div className="mt-8 pt-6 border-t border-slate-200 dark:border-slate-800/50 space-y-3">
                <h5 className="text-[8px] font-black text-slate-500 dark:text-slate-700 uppercase tracking-[0.2em] px-2 mb-2">Recently Finished</h5>
                {completedPrereqs.map(p => (
                  <div key={p.id} className="flex items-center justify-between gap-4 px-4 py-3 bg-slate-50 dark:bg-black/40 rounded-2xl border border-slate-200 dark:border-slate-800/50 hover:border-emerald-500/30 transition-all relative overflow-visible">
                    <span className="text-xs font-bold text-slate-400 dark:text-slate-600 line-through truncate max-w-[200px]">{p.label || "Unnamed Energy"}</span>
                    <button 
                      onClick={(e) => handlePrereqToggle(e, p.id)}
                      ...
                      <div className="absolute inset-0 flex items-center justify-center pointer-events-none overflow-visible z-50">
                        {popups.filter(pop => pop.type === 'prereq' && pop.prereqId === p.id).map(pop => (
                          ...
                          >
                            ...
                            <div className="text-amber-500 dark:text-amber-400 text-[10px] font-black flex items-center gap-1">{pop.coinAmount > 0 ? `+${pop.coinAmount}` : pop.coinAmount} ₵</div>
                          </div>
                        ))}
                      </div>
