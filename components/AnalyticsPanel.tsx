...
const AnalyticsPanel: React.FC<AnalyticsPanelProps> = ({ tasks, state }) => {
  ...
  if (completionRate > 25) { resilienceLevel = "Growing"; resilienceColor = "text-blue-500 dark:text-blue-400"; }
  if (completionRate > 50) { resilienceLevel = "Strong"; resilienceColor = "text-indigo-500 dark:text-indigo-400"; }
  if (completionRate > 75) { resilienceLevel = "Unshakeable"; resilienceColor = "text-emerald-500 dark:text-emerald-400"; }
  ...
  return (
    <div className="grid md:grid-cols-2 gap-8 pb-24 relative z-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="md:col-span-2 bg-white dark:bg-[#111214] border-2 border-slate-200 dark:border-slate-800 p-8 md:p-10 rounded-[3rem] shadow-2xl relative overflow-hidden group hover:border-emerald-500/20 transition-all duration-500">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-10">
          <div className="text-center md:text-left space-y-4">
            <div>
              <h3 className="text-emerald-600 dark:text-emerald-500 font-black text-[10px] uppercase tracking-[0.4em] mb-2">Mastery Status</h3>
              <h2 className="text-4xl md:text-5xl font-black text-slate-900 dark:text-white uppercase tracking-tighter">{state.rankTitle}</h2>
            </div>
      ...
      <div className="bg-white dark:bg-[#111214] p-8 rounded-[3rem] border border-slate-200 dark:border-slate-800 shadow-xl flex flex-col justify-between">
        <h3 className="text-lg font-black text-slate-900 dark:text-white mb-8 tracking-tighter uppercase flex items-center gap-2">
          ...
          {categoryData.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={categoryData} barSize={20}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke={state.theme === 'light' ? '#e2e8f0' : '#1f2937'} />
                <XAxis 
                  dataKey="name" 
                  axisLine={false} 
                  tickLine={false} 
                ...
                />
                <Tooltip 
                  cursor={{ fill: state.theme === 'light' ? '#f1f5f9' : '#1f2937', radius: 4 }}
                  contentStyle={{ 
                    backgroundColor: state.theme === 'light' ? '#ffffff' : '#0f172a', 
                    borderRadius: '12px', 
                    border: state.theme === 'light' ? '1px solid #e2e8f0' : '1px solid #1e293b', 
                    boxShadow: '0 10px 30px -10px rgba(0,0,0,0.5)', 
                    fontSize: '12px', 
                    fontWeight: 'bold',
                    color: state.theme === 'light' ? '#0f172a' : '#e2e8f0'
                  }}
                  itemStyle={{ color: state.theme === 'light' ? '#0f172a' : '#e2e8f0' }}
                />
      ...
      {/* 3. EQUILIBRIUM INDEX */}
      <div className="bg-white dark:bg-[#111214] p-8 rounded-[3rem] border border-slate-200 dark:border-slate-800 shadow-xl flex flex-col items-center relative overflow-hidden">
        <div className="w-full flex justify-between items-start mb-4 relative z-10">
           <h3 className="text-lg font-black text-slate-900 dark:text-white tracking-tighter uppercase flex items-center gap-2">
      ...
      <div className="md:col-span-2 bg-white dark:bg-[#111214] text-slate-900 dark:text-white p-10 md:p-12 rounded-[4rem] border-2 border-slate-200 dark:border-slate-800 relative overflow-hidden shadow-2xl">
        <div className="relative z-10 flex flex-col md:flex-row items-center gap-12">
           <div className="flex-1 text-center md:text-left">
              <h3 className="text-2xl font-black mb-4 tracking-tighter uppercase text-slate-800 dark:text-slate-200">Chronicle of Balance</h3>
              <p className="text-slate-500 dark:text-slate-400 font-bold leading-loose text-lg italic">
                "{getNarrative()}"
              </p>
           </div>
           
           <div className="flex-shrink-0 grid grid-cols-2 gap-4 w-full md:w-auto">
              <div className="p-6 bg-slate-50 dark:bg-black/40 rounded-[2rem] border border-slate-200 dark:border-slate-800 text-center backdrop-blur-sm">
                 <span className="text-[9px] text-slate-500 dark:text-slate-600 font-black uppercase tracking-widest block mb-2">Resilience</span>
                 <span className={`text-xl font-black uppercase ${resilienceColor}`}>{resilienceLevel}</span>
              </div>
              <div className="p-6 bg-slate-50 dark:bg-black/40 rounded-[2rem] border border-slate-200 dark:border-slate-800 text-center backdrop-blur-sm">
                 <span className="text-[9px] text-slate-500 dark:text-slate-600 font-black uppercase tracking-widest block mb-2">Clutter</span>
        ...
        {/* Ambient Effects */}
        <div className="absolute top-0 right-0 w-[400px] h-[400px] bg-indigo-600/5 dark:bg-indigo-600/10 rounded-full blur-[100px] -mr-40 -mt-40 pointer-events-none"></div>
        <div className="absolute bottom-0 left-0 w-[300px] h-[300px] bg-emerald-600/5 dark:bg-emerald-600/10 rounded-full blur-[100px] -ml-40 -mb-40 pointer-events-none"></div>
      </div>
    </div>
  );
};
