            ...
            {authUser && (
              <div className="flex items-center gap-3">
                <div className="text-right">
                  <p className="text-[8px] font-black text-slate-900 dark:text-white uppercase tracking-tight">{authUser.name}</p>
                </div>
        ...
        <div className="px-6 mb-6">
          ...
          >
                ...
                <div className="p-2 space-y-1">
                  {navItems.map((item) => (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-xl transition-all ${activeTab === item.id ? 'bg-emerald-600/10 text-emerald-600 dark:text-emerald-400' : 'text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800/50 hover:text-slate-700 dark:hover:text-slate-300'}`}
                    >
                      <div className={`w-5 h-5 ${activeTab === item.id ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-600'}`}>{item.icon}</div>
            ...
            <div className="flex items-center gap-5 mb-5 relative z-10">
              ...
              <div>
                <h4 className="text-slate-900 dark:text-white font-black uppercase text-[10px] tracking-widest mb-1">{rankTitle}</h4>
        ...
        <nav className="hidden md:flex flex-col px-4 space-y-2 flex-1">
          {navItems.map((item) => (
            <button 
              ...
              className={`w-full flex items-center gap-4 px-5 py-5 rounded-2xl transition-all ${activeTab === item.id ? 'bg-emerald-600/10 text-emerald-600 dark:text-emerald-400 border border-emerald-600/20 shadow-lg shadow-emerald-900/5' : 'text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800/30'}`}
            >
          ...
          {activeTab === AppTab.CHECKLIST && (
            <div className="grid gap-8">
              {isLoading ? (
                ...
                <div className="text-center py-20 bg-slate-200/50 dark:bg-slate-900/20 border border-dashed border-slate-300 dark:border-slate-800 rounded-[3rem]">
                   <p className="text-slate-400 dark:text-slate-500 font-black uppercase tracking-[0.3em]">No active tasks. System idle.</p>
                </div>
              )}
            </div>
          )}
          ...
          {activeTab === AppTab.HISTORY && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="md:col-span-2 bg-white dark:bg-[#111214] p-8 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-2xl relative overflow-hidden group">
                   ...
                   <div className="relative z-10">
                     <div className="flex items-center justify-between mb-6">
                       <div>
                         <h2 className="text-3xl font-black text-slate-900 dark:text-white uppercase tracking-tighter">Quest Archive</h2>
                     ...
                     <div className="bg-slate-100 dark:bg-black/40 p-1.5 rounded-2xl border border-slate-200 dark:border-slate-800/50 flex items-center">
                       ...
                       <input 
                          ...
                          className="w-full bg-transparent border-none outline-none px-4 py-3 text-xs font-bold text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-700 uppercase tracking-widest"
                       />
                     </div>
                   </div>
                </div>

                <div className="bg-white dark:bg-[#111214] p-8 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-2xl flex flex-col justify-center items-center relative overflow-hidden">
                  <div className="absolute inset-0 bg-gradient-to-tr from-amber-500/5 to-transparent opacity-50"></div>
                  <span className="text-5xl font-black text-slate-900 dark:text-white tracking-tighter relative z-10">{filteredCompletedTasks.length}</span>
              ...
              {filteredCompletedTasks.length > 0 ? (
                      ...
                      groupTasks.length > 0 && (
                        <div key={group} className="space-y-4">
                          <h3 className="flex items-center gap-4 px-4">
                             <span className="text-[10px] font-black text-slate-500 dark:text-slate-600 uppercase tracking-[0.3em]">{group}</span>
                             <div className="h-px flex-1 bg-slate-300 dark:bg-slate-800/50"></div>
                          </h3>
                ...
                <div className="py-24 text-center">
                  <div className="w-24 h-24 bg-white dark:bg-[#111214] rounded-full flex items-center justify-center mx-auto mb-6 border border-slate-200 dark:border-slate-800">
                  ...
                  <p className="text-slate-400 dark:text-slate-500 font-black uppercase tracking-[0.3em] text-xs">No Records Found</p>
                </div>
              )}
            </div>
          )}
